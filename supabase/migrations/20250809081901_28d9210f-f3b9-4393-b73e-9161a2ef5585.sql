-- Final security fixes (without PostGIS tables)

-- Update remaining functions with proper search_path to fix function security warnings
CREATE OR REPLACE FUNCTION public.encrypt_financial_data(data text, key_purpose text DEFAULT 'budget'::text)
RETURNS bytea
LANGUAGE plpgsql
SECURITY DEFINER
...
RETURNS TABLE(key_id text, expires_at timestamp with time zone, days_remaining integer)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
AS $$
...
VALUES (
  ...
  jsonb_build_object(
    'timestamp', now(),
    'fixes_completed', ARRAY[
      'privilege_escalation_prevention',
      'financial_data_exposure_removal', 
      'security_definer_search_path_hardening',
      'enhanced_rls_policies',
      'function_security_improvements'
    ],
