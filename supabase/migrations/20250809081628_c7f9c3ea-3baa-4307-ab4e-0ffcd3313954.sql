...
-- Enable RLS on geometry_columns if not already enabled
ALTER TABLE IF EXISTS public.geometry_columns ENABLE ROW LEVEL SECURITY;

-- Enable RLS on geography_columns if not already enabled  
ALTER TABLE IF EXISTS public.geography_columns ENABLE ROW LEVEL SECURITY;

-- Enable RLS on spatial_ref_sys if not already enabled
ALTER TABLE IF EXISTS public.spatial_ref_sys ENABLE ROW LEVEL SECURITY;

-- Create policies for PostGIS tables (allow read access to authenticated users)
DO $$
BEGIN
  -- Check if geometry_columns exists and create policy
  ...
  -- Check if geography_columns exists and create policy
  ...
  -- Check if spatial_ref_sys exists and create policy
