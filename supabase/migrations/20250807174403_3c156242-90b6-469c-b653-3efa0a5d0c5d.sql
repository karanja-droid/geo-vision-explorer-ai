-- ============================================================================
-- GEOVISION AI MINER SECURITY ENHANCEMENTS (CORRECTED)
-- ============================================================================
...
-- 8. Enhanced RLS policies for projects (using correct enum values)
...
FOR SELECT USING (
  EXISTS (
    ...
    AND (p.owner_id = auth.uid() OR get_current_user_role() IN ('admin', 'geologist'))
  )
);
...
FOR UPDATE USING (
  get_current_user_role() IN ('qa_qc', 'admin')
...
SELECT 
  ...
  CASE 
    WHEN get_current_user_role() IN ('admin', 'executive') 
