-- Enhanced encryption key management system
CREATE TABLE IF NOT EXISTS public.encryption_keys (
  ...
  rotation_status TEXT DEFAULT 'active' CHECK (rotation_status IN ('active', 'pending_rotation', 'rotated', 'expired')),
...
-- Only system and admins can manage encryption keys
CREATE POLICY "System can manage encryption keys" ON public.encryption_keys
FOR ALL USING (true);

CREATE POLICY "Admins can view encryption keys metadata" ON public.encryption_keys
FOR SELECT USING (get_current_user_role() = 'admin');

-- Enhanced encryption function with key versioning
...
BEGIN
  ...
  IF current_key_id IS NULL THEN
    ...
    INSERT INTO encryption_keys (key_id, encrypted_key, expires_at, metadata)
    VALUES (
      current_key_id,
...
-- Function to check if keys need rotation
...
-- Enhanced key rotation function
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      INSERT INTO encryption_keys (key_id, encrypted_key, expires_at, key_version, metadata)
      VALUES (
        new_key_id,
      ...
      IF old_key.metadata->>'purpose' = 'budget' THEN
        UPDATE projects 
        SET budget_encrypted = encrypt_financial_data(
          decrypt_financial_data(budget_encrypted),
          'budget'
        )
...
-- Trigger function to automatically encrypt budget data
...
BEGIN
  ...
  IF NEW.budget IS NOT NULL AND NEW.budget_encrypted IS NULL THEN
    NEW.budget_encrypted := encrypt_financial_data(NEW.budget::text, 'budget');
...
-- Create trigger for automatic budget encryption
DROP TRIGGER IF EXISTS auto_encrypt_budget_trigger ON projects;
CREATE TRIGGER auto_encrypt_budget_trigger
  BEFORE INSERT OR UPDATE ON projects
  FOR EACH ROW
  EXECUTE FUNCTION auto_encrypt_budget();

-- Function to get decrypted budget (for authorized users only)
...
BEGIN
  ...
  decrypted_value := decrypt_financial_data(encrypted_budget);
...
CREATE INDEX IF NOT EXISTS idx_encryption_keys_active ON encryption_keys(is_active, expires_at) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_encryption_keys_purpose ON encryption_keys USING gin(metadata) WHERE is_active = true;